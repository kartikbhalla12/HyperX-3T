...
int btrfsic_submit_bh(int rw, struct buffer_head *bh)
{
	...
	if (NULL != dev_state &&
		...
		if (dev_state->state->print_mask &
			...
			pr_info("submit_bh(rw=0x%x, blocknr=%llu (bytenr %llu), size=%zu, data=%p, bdev=%p)\n",
...
static void __btrfsic_submit_bio(int rw, struct bio *bio)
{
	...
	if (NULL != dev_state &&
		...
		if (dev_state->state->print_mask &
		    BTRFSIC_PRINT_MASK_SUBMIT_BIO_BH)
			pr_info("submit_bio(rw=0x%x, bi_vcnt=%u, bi_sector=%llu (bytenr %llu), bi_bdev=%p)\n",
